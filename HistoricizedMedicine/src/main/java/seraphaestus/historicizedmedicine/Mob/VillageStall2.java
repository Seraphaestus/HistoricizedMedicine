...
public class VillageStall2 extends Village {

	private EnumFacing facing;
	private Random rand;
	private int groundLevel = -1;

	public VillageStall2() {
		this.facing = EnumFacing.NORTH;
	}

	public VillageStall2(Start villagePiece, int par2, Random par3Random, StructureBoundingBox par4StructureBoundingBox, EnumFacing facing) {
	...
	protected boolean placeChest(World world, StructureBoundingBox box, Random rand, int x, int y, int z) {
		...
		{
			world.setBlockState(pos, Blocks.CHEST.getDefaultState().withProperty(BlockChest.FACING, EnumFacing.byIndex(4)));
	...
	public static class VillageManager implements IVillageCreationHandler {
		...
		public Village buildComponent(PieceWeight villagePiece, Start startPiece, List<StructureComponent> pieces, Random random, int p1, int p2, int p3, EnumFacing facing, int p5) {
			...
			return (!canVillageGoDeeper(box)) || (StructureComponent.findIntersecting(pieces, box) != null) ? null : new VillageStall2(startPiece, p5, random, box, facing);
		}

		@Override
		public PieceWeight getVillagePieceWeight(Random random, int i) {
			return new PieceWeight(VillageStall2.class, 30, MathHelper.getInt(random, 0 + i, 1 + i));
		}

		@Override
		public Class<?> getComponentClass() {
			return VillageStall2.class;
		}
	}
}
